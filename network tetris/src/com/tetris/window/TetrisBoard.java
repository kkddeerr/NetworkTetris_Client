...
public class TetrisBoard extends JPanel implements Runnable, KeyListener, MouseListener, ActionListener{
	...
	public static final int CREATE_ITEM = 1000;//아이템이 생기는 주기 100=1초
	...
	public TetrisBoard(Tetris tetris, GameClient client) {
		this.tetris = tetris;
		this.client = client;
		this.setPreferredSize(new Dimension(PANEL_WIDTH+440,PANEL_HEIGHT));//기본크기
	...
	protected void paintComponent(Graphics g) {
		...
		g.fillRect(BOARD_X + BLOCK_SIZE*minX, BOARD_Y, maxX*BLOCK_SIZE+1, maxY*BLOCK_SIZE+1);//자신의 테트리스 화면
		
		g.fillRect(350+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y, maxX*SMALL_BLOCK_SIZE+1, maxY*SMALL_BLOCK_SIZE+1);//유저1의 테트리스 화면
		g.drawString("1", 350+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y+243);
		g.fillRect(493+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y, maxX*SMALL_BLOCK_SIZE+1, maxY*SMALL_BLOCK_SIZE+1);//유저2의 테트리스 화면
		g.drawString("2", 493+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y+243);
		g.fillRect(636+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y, maxX*SMALL_BLOCK_SIZE+1, maxY*SMALL_BLOCK_SIZE+1);//유저3의 테트리스 화면
		g.drawString("3", 636+BOARD_X + BLOCK_SIZE*minX+4, BOARD_Y+243);
		g.fillRect(350+BOARD_X + BLOCK_SIZE*minX+4, 275+BOARD_Y, maxX*SMALL_BLOCK_SIZE+1, maxY*SMALL_BLOCK_SIZE+1);//유저4의 테트리스 화면
		g.drawString("4", 350+BOARD_X + BLOCK_SIZE*minX+4, 275+BOARD_Y+243);
		g.fillRect(493+BOARD_X + BLOCK_SIZE*minX+4, 275+BOARD_Y, maxX*SMALL_BLOCK_SIZE+1, maxY*SMALL_BLOCK_SIZE+1);//유저5의 테트리스 화면
				...
				for(int j=0;j<maxX;j++){
					if(itemList.size()<10){
						if(map[mainBlock.getY()][j].getItem()!=0){
							...
							itemList.add(new Item(map[mainBlock.getY()][j].getItem()));//아이템리스트에 아이템이 추가됨
							if(!itemList.isEmpty())
	...
	public void usingItem(){
		if(!itemList.isEmpty()){
			if(itemList.get(0).getItemNum()==1){//clear실행
				for(int i=0;i<maxY;i++)
					removeBlockLine(maxY-1);
			}
			else if(itemList.get(0).getItemNum()==2){//plus1실행
				addBlockLine(1);
			}
			else if(itemList.get(0).getItemNum()==3){//minus1실행
				removeBlockLine(maxY-1);
			}
			itemList.remove(0);
			for(int i=0;i<9;i++){
	...
	public void usingItem(int itemNum){
		if(itemNum==1){//clear실행
			for(int i=0;i<maxY;i++)
				removeBlockLine(maxY-1);
		}
		else if(itemNum==2){//plus1실행
			addBlockLine(1);
		}
		else if(itemNum==3){//minus1실행
			removeBlockLine(maxY-1);
		}
	}
